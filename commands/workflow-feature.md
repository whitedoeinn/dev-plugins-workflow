---
description: Feature workflow - exploration by default, patterns when detected
---
...
```bash
/wdi:workflow-feature                     # Start new (interactive)
/wdi:workflow-feature #45                 # Continue existing issue
/wdi:workflow-feature --quick             # Skip 3-lens exploration (trivial fixes)
/wdi:workflow-feature --headless          # No prompts, agent mode
/wdi:workflow-feature --stop-after=plan   # Stop after phase
/wdi:workflow-feature --idea "text"       # Quick capture
```

## Philosophy

Traditional workflow was too rigid. Exploration deserves to be the default, not a separate command.

**Default:** 3-lens exploration (Conservative/Balanced/Radical) before building.
**Escape hatch:** `--quick` for trivial fixes where exploration is overkill.
...
| Flag | Purpose |
|------|---------|
| `--quick` | Direct to work (no 3-lens exploration) |
...
**HARD GATE: Stop here. Do not proceed until human curates.**
...
```bash
/wdi:workflow-feature #45 --headless
/wdi:workflow-feature #45 --headless --quick
/wdi:workflow-feature #45 --headless --stop-after=work
```
