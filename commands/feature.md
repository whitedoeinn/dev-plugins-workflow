...
# /claude-workflows:feature - Compound Engineering Feature Workflow
...
---
Generated by /claude-workflows:feature workflow
EOF
)"
```
...
Invoke existing `/claude-workflows:commit` workflow for changelog:
...
```
/claude-workflows:feature --yes Add print button to expense form
```
...
```
/claude-workflows:feature --plan-only Implement user dashboard with analytics
```
...
```
/claude-workflows:feature --skip-research Fix navigation styling on mobile
```
...
```
/claude-workflows:feature Add dark mode toggle to settings
...
- Integrates with existing `/claude-workflows:commit` for changelog updates
